...
extern wchar_t const __declspec(selectany) c_szARDroneScheme[] = L"ardrone";
extern wchar_t const __declspec(selectany) c_szARDroneSchemeWithColon[] = L"ardrone:";
...
class CARDroneMediaSource
    : public OpQueue<CARDroneMediaSource, CSourceOperation>
    , public IMFMediaSourceEx
    , public IMFGetService
    , public IMFRateControl
{
public:
    static HRESULT CreateInstance(CARDroneMediaSource**ppSource);
...
private:
    ...
    HRESULT ParseServerUrl(LPCWSTR pszUrl, LPCWSTR *pShape);
...
private:
    ...
    ComPtr<CARDroneMediaStream>       _spStream;                // We only have one stream
    ...
    AsyncCallback<CARDroneMediaSource> _OnFinishOpenCB;         
};
