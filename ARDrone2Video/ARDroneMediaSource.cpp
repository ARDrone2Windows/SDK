...
CARDroneMediaSource::CARDroneMediaSource(void)
    : OpQueue<CARDroneMediaSource, CSourceOperation>(_critSec.m_criticalSection)
    , _cRef(1)
    , _eSourceState(SourceState_Invalid)
    , _flRate(1.0f)
#pragma warning(push)
#pragma warning(disable: 4355)
    , _OnFinishOpenCB(this, &CARDroneMediaSource::OnFinishOpen)
#pragma warning(pop)
{
...
HRESULT CARDroneMediaSource::CreateInstance(CARDroneMediaSource **ppSource)
{
    HRESULT hr = S_OK;
    ...
    spSource.Attach(new(std::nothrow) CARDroneMediaSource());
    if (!spSource)
    {
        hr = E_OUTOFMEMORY;
    }
...
IFACEMETHODIMP CARDroneMediaSource::QueryInterface(REFIID riid, void** ppv)
{
    if (ppv == nullptr)
    {
        return E_POINTER;
...
IFACEMETHODIMP_(ULONG) CARDroneMediaSource::AddRef()
{
    return InterlockedIncrement(&_cRef);
}

IFACEMETHODIMP_(ULONG) CARDroneMediaSource::Release()
{
    long cRef = InterlockedDecrement(&_cRef);
    if (cRef == 0)
    {
...
IFACEMETHODIMP CARDroneMediaSource::BeginGetEvent(IMFAsyncCallback* pCallback, IUnknown* punkState)
{
    HRESULT hr = S_OK;
...
IFACEMETHODIMP CARDroneMediaSource::EndGetEvent(IMFAsyncResult* pResult, IMFMediaEvent** ppEvent)
{
    HRESULT hr = S_OK;
...
IFACEMETHODIMP CARDroneMediaSource::GetEvent(DWORD dwFlags, IMFMediaEvent** ppEvent)
{
    // NOTE:
...
IFACEMETHODIMP CARDroneMediaSource::QueueEvent(MediaEventType met, REFGUID guidExtendedType, HRESULT hrStatus, PROPVARIANT const *pvValue)
{
    HRESULT hr = S_OK;
...
IFACEMETHODIMP CARDroneMediaSource::CreatePresentationDescriptor(
...
IFACEMETHODIMP CARDroneMediaSource::GetCharacteristics(DWORD* pdwCharacteristics)
{
...
IFACEMETHODIMP CARDroneMediaSource::Pause()
{
    return MF_E_INVALID_STATE_TRANSITION;
}

IFACEMETHODIMP CARDroneMediaSource::Shutdown()
{
    AutoLock lock(_critSec);
...
IFACEMETHODIMP CARDroneMediaSource::Start(
...
IFACEMETHODIMP CARDroneMediaSource::Stop()
{
    HRESULT hr = S_OK;
...
// IMFMediaSourceEx
IFACEMETHODIMP CARDroneMediaSource::GetSourceAttributes(_Outptr_ IMFAttributes **ppAttributes)
{
...
IFACEMETHODIMP CARDroneMediaSource::GetStreamAttributes(_In_ DWORD dwStreamIdentifier, _Outptr_ IMFAttributes **ppAttributes)
{
    if (ppAttributes == nullptr)
    {
        return E_POINTER;
...
IFACEMETHODIMP CARDroneMediaSource::SetD3DManager(_In_opt_ IUnknown *pManager)
{
    HRESULT hr = CheckShutdown();
    if (SUCCEEDED(hr))
    {
...
IFACEMETHODIMP CARDroneMediaSource::GetService( _In_ REFGUID guidService, _In_ REFIID riid, _Out_opt_ LPVOID *ppvObject)
{
...
IFACEMETHODIMP CARDroneMediaSource::SetRate(BOOL fThin, float flRate)
{
    if (fThin)
    {
...
IFACEMETHODIMP CARDroneMediaSource::GetRate(_Inout_opt_ BOOL *pfThin, _Inout_opt_ float *pflRate)
{
    AutoLock lock(_critSec);
...
__override HRESULT CARDroneMediaSource::DispatchOperation(CSourceOperation *pOp)
{
    AutoLock lock(_critSec);
...
__override HRESULT CARDroneMediaSource::ValidateOperation(CSourceOperation *pOp)
{
    return S_OK;
}

HRESULT CARDroneMediaSource::BeginOpen(LPCWSTR pszUrl, IMFAsyncCallback *pCB, IUnknown *pUnkState)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = ParseServerUrl(pszUrl, &hostName);
    }
    
    if (SUCCEEDED(hr))
    {
        hr = CARDroneMediaStream::CreateInstance(this, hostName, &_spStream);
    }
...
HRESULT CARDroneMediaSource::EndOpen(IMFAsyncResult *pResult)
{
...
HRESULT CARDroneMediaSource::Initialize()
{
    HRESULT hr = S_OK;
    ...
    if (SUCCEEDED(hr))
    {
		...
		_spAttributes->SetUINT32(MF_LOW_LATENCY, TRUE);
    }
...
// Handle errors
void CARDroneMediaSource::HandleError(HRESULT hResult)
{
...
HRESULT CARDroneMediaSource::CompleteOpen(HRESULT hResult)
{
...
HRESULT CARDroneMediaSource::DoStart(CStartOperation *pOp)
{
...
HRESULT CARDroneMediaSource::DoStop(CSourceOperation *pOp)
{
...
HRESULT CARDroneMediaSource::DoSetRate(CSetRateOperation *pOp)
{
...
HRESULT CARDroneMediaSource::ValidatePresentationDescriptor(IMFPresentationDescriptor *pPD)
{
    HRESULT hr = S_OK;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr) && dwId != c_dwARDroneStreamId)
            {
                hr = E_INVALIDARG;
            }
        }
...
HRESULT CARDroneMediaSource::ParseServerUrl(LPCWSTR pszUrl, LPCWSTR *hostName)
{
    if (pszUrl == nullptr || hostName == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr) && (pszScheme == nullptr || wcscmp(pszScheme, c_szARDroneScheme) != 0))
        {
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            *hostName = WindowsGetStringRawBuffer(strHost.Get(), nullptr);
            hr = S_OK;            
        }
    }
...
HRESULT CARDroneMediaSource::OnFinishOpen(IMFAsyncResult *pResult)
{
    HRESULT hr = pResult->GetStatus();
...
BOOL CARDroneMediaSource::IsRateSupported(float flRate, float *pflAdjustedRate)
{
